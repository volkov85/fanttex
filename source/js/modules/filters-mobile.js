...
(function () {
  function initFilters() {
    ...
    if (selectionPrice) {
      ...
      dropdownInit(selectionPrice, false, selectionPriceInputMin, selectionPriceInputMax);
    ...
    for (let i = 0; i < selections.length; i++) {
      ...
      dropdownInit(selections[i], false, selectionSearchInput, ...Array.from(selectionOptions).map((option) => {
      ...
      selectionOpen.addEventListener(`click`, (evt) => {
        ...
        selections[i].classList.toggle(`opened`);
      });
    ...
    /**
     * FILTERS BUTTON
     * Оживление кнопки "Фильтры" для мобильных
     */
    let fultersNavOpenBtn = document.querySelector(`.filters__mobile-open`);

    if (fultersNavOpenBtn) {
      ...
      window.addEventListener(`scroll`, () => {
        if (!document.body.classList.contains(`page--block`)) {
          ...
          if (currentScrollPos > prevScrollPos) {
            if (fultersNavOpenBtn.classList.contains(`shown`)) {
              fultersNavOpenBtn.classList.remove(`shown`);
            }
          } else {
            fultersNavOpenBtn.classList.add(`shown`);
          }
