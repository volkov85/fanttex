 ...
 */
function dropdownInit(dropdownEl, ...dropdownFocusedEls) {
  ...
  for (let i = 0; i < dropdownFocusedEls.length; i++) {
    ...
    dropdownFocusedEls[i].addEventListener(`blur`, () => {
      ...
      if (!dropdownEl.classList.contains(`hovered`)) {
        dropdownEl.classList.remove(`opened`);

        dropdownEl.firstChild.lastChild.style.removeProperty(`left`);
      }
    });
  }
}
...
/**
 * Сдвигает выпадающий список, если он выходит за пределы экрана
 */
function shiftDropdownList(dropdownEl, dropdownListWidth) {
  let pageWidth = document.documentElement.clientWidth;
  let dropdownList = dropdownEl.firstChild.lastChild;

  dropdownList.style.width = 0;
  dropdownEl.firstChild.lastChild.style.removeProperty(`left`);
  dropdownEl.classList.toggle(`opened`);

  let elListHorizontalOffset = (pageWidth - 15) - (Math.floor(dropdownList.getBoundingClientRect().left) + dropdownListWidth);

  if (elListHorizontalOffset < 0) {
    dropdownList.style.left = `${elListHorizontalOffset}px`;
  }

  dropdownList.style.removeProperty(`width`);
}

/**
 * Листает слайдер в зависимости от положения мыши
 */
function swipeSlidesOnMouseMove(preview, swiper) {
  let oldSlideIndex = 0;

  let productCardPreviewRect = preview.getBoundingClientRect();
  let activeAreaWidth = productCardPreviewRect.width / swiper.slides.length;

  preview.addEventListener(`mousemove`, (evt) => {
    let mousePosX = evt.clientX - productCardPreviewRect.left;
    let slideIndex = Math.floor(mousePosX / activeAreaWidth);

    if (slideIndex >= 0 || slideIndex !== oldSlideIndex) {
...
function changeProductPriceValue(priceItem, price) {
  ...
  let makeCounterEffect = (startValue, endValue, duration) => {
    ...
    let stepValue = Math.floor((endValue - startValue) / 100);
